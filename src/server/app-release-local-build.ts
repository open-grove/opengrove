...
export function saveMountedAppReleasePrebuildDraft(input: {
  ...
  return withReleaseInstallGenerationLock({ installContainerRoot }, () => {
    ...
    const sourceManifest = frozenSource ? readMountedAppManifest(sourceRoot).manifest : input.target.manifest;
    if (!sourceManifest) throw new Error("app_revision_manifest_invalid");
    const sourceTarget: MountedAppTarget = { ...input.target, appRoot: sourceRoot, manifest: sourceManifest };
    ...
    const workingState = appWorkingTreeStateForRoot(
      sourceRoot,
      mountedAppWorkingManifest(input.state, sourceTarget),
    ...
    const draft = saveMountedAppDraft({
      ...
      ...(frozenSource
        ? {
            ...
            manifestOverride: sourceManifest as OpenGroveAppManifest,
            workingContentDigestOverride: workingState.digest,
          }
        : {}),
...
function saveBuiltReleaseDraft(input: {
  ...
  try {
    ...
    return withReleaseInstallGenerationLock(input.installFence, () => {
      ...
      input.budget.checkpoint();
      const builtManifest = input.sourceBoundToSavePoint
        ? readMountedAppManifest(input.builtAppRoot).manifest
        : input.target.manifest;
      if (!builtManifest) throw new Error("app_revision_manifest_invalid");
      const saved = saveMountedAppDraftForRelease({
        state: input.state,
        target: { ...input.target, manifest: builtManifest },
        release: input.release,
        store: input.draftStore,
