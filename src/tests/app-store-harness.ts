...
try {
  ...
  const failedDefaultRevisionRoot = managedAppRevisionGitDirectory(
    join(appStoreDataRoot(failedDefaultActivationState), "app-revisions"),
    "story-seed",
  );
  assert.equal(existsSync(failedDefaultRevisionRoot), false);
  ...
  assert.equal(
    existsSync(failedDefaultRevisionRoot),
    false,
    "failed default activation must remove its new source repository",
  );
    ...
    try {
      ...
      assert.equal(
        await dispatchBridgeRoutes(createBridgeRoutes(), {
          ...
          url: new URL("http://opengrove.test/apps/first-publish-app/publish"),
          traceId: "trace-app-release-linked-source",
          state: firstPublishState,
          security: adminSessionSecurity,
          sendJson: captureReleaseResponse(linkedPublishCalls),
          readJsonBody: async () => ({
            ...gitPrepareCalls[0]?.data.release,
  ...
  const failedEmployeeState = createBridgeState({ statePath: join(tempRoot, "employee-failed-state.json") });
  const failedEmployeeCalls: ReleaseHarnessCall[] = [];
  let failEmployeeActivation = true;
  await handleAppStoreRoute({
    request: { method: "POST", headers: { cookie: "sample_cloud_session=harness" } } as IncomingMessage,
    response: {} as ServerResponse,
    url: new URL("http://opengrove.test/app-store/install"),
    state: failedEmployeeState,
    security: sessionSecurity,
    sendJson: captureReleaseResponse(failedEmployeeCalls),
    readJsonBody: async () => ({ packageId: employeeManifest.id }),
    activateBridgeApp: (state, options) => {
      if (failEmployeeActivation) {
        failEmployeeActivation = false;
        throw new Error("injected_employee_activation_failure");
      }
      recreateBridgeApp(state, options);
    },
  });
  assert.equal(failedEmployeeCalls[0]?.status, 500);
  assert.equal(
    failedEmployeeState.app.rooms.listMembers().some((member) => member.storePackageId === employeeManifest.id),
    false,
    "failed employee activation must restore the pre-install member snapshot",
  );
  await failedEmployeeState.store.close?.();
