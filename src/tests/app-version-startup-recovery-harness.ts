...
try {
  ...
  const unrelatedApp = {
    id: "concurrent-app",
    path: join(tempRoot, "concurrent-app"),
    enabled: false,
    title: "Concurrent App",
  };
  state.settings.mountedApps.push(unrelatedApp);
  state.settings.uninstalledStoreAppIds.push("another-uninstalled-app");
  ...
  state = createBridgeState({ statePath });
  assert.deepEqual(
    state.settings.mountedApps.find((app) => app.id === "concurrent-app"),
    unrelatedApp,
  );
  assert.ok(state.settings.uninstalledStoreAppIds.includes("another-uninstalled-app"));
