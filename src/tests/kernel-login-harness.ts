...
`,
        ...
        for (const [name, output, expected] of [
          [
            "bedrock",
            JSON.stringify({ loggedIn: true, authMethod: "third_party", apiProvider: "bedrock" }),
            "provider",
          ],
...
`,
          ...
          state.settings.kernelPathOverrides["claude-code"] = {
            binaryPath: command,
            configHome: join(root, `claude-${name}-home`),
          };
          const login = (await describeKernelLogins(state)).find((item) => item.kernelId === "claude-code");
          assert.equal(login?.status, expected, `Claude ${name} authentication must be classified consistently`);
          assert.equal(
            kernelLoginRouteProfiles(state).some((route) => route.sourceKernel === "claude-code"),
            expected === "authenticated",
          );
          if (name === "bedrock") assert.equal(login?.providerId, "aws-bedrock-api-key");
