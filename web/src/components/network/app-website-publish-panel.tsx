...
export function AppWebsitePublishPanel({ appId }: { appId: string }) {
  ...
  const prepare = useMutation({
    ...
    mutationFn: async () => {
      ...
      if (result.website.reviewStatus !== "current") await sendReview(result.website);
    },
  });
  ...
  const configure = useMutation({
    ...mutationFeedback,
    mutationFn: async () => {
      const policy: AppWebsiteConfig["audience"] =
        audience === "admin"
          ? { mode: "roles", roles: ["admin"] }
          : audience === "roles"
            ? { mode: "roles", roles: roles.split(/[,，\s]+/).filter(Boolean) }
            : { mode: audience === "public" ? "public" : "authenticated" };
      const result = await openGroveClient.apps.website.configure({ appId, audience: policy });
      update(result.website);
      setAudience("");
      setReviewSent(false);
    },
