...
assert.equal(harnessInventory.length, 115, "the canonical deterministic harness inventory must not shrink silently");
assert.equal(
  harnessGroups.full,
  harnessInventory,
  "the full group should be the canonical inventory, not a second list",
);
assert.equal(new Set(harnessInventory.map((task) => task.id)).size, 115, "every harness id must be unique");
assert.equal(harnessGroups.integration.length, 40, "the affected-integration subset must not shrink silently");
assert.equal(
