...
的路径，刷新时重新扫描以发现更新后的目录。临时验证失败保留上次验证成功的路径，
读取时仍排除文件已被删除的旧路径。仍无可用候选时，查询已注册的
...
每个状态写入进程在独立的空文件 `<state-path>.lock.sqlite` 上持有 SQLite
排他事务，用操作系统文件锁保护整个写入进程的生命周期。进程退出或崩溃后，
操作系统会释放锁；业务数据库不因这层保护而保持长事务。可能还有写入进程
正在运行或启动时，不能删除或替换这个协调文件。

相邻的 `.lock` JSON 标记用于兼容 OpenGrove 0.6.6 及更早版本。新标记带有
`ownershipProtocol: "sqlite-v1"`，获取操作系统锁后即可清理遗留的新标记。
旧标记只有在同一台机器上且原进程已退出，或启动时间证明进程编号已被复用时
才会恢复；查询不到进程信息不能证明旧写入者已退出。桌面恢复异步查询进程，
同次恢复中相同进程编号只查询一次，Windows 查询允许八秒启动时间。

桌面关闭时会等待 Bridge 真正退出，再允许启动替代进程。关闭失败会保留进程
信息并提供停止、重试操作；退出桌面也会取消尚未完成的启动和崩溃重启任务。
桌面所属 Bridge 在父进程断开 IPC 后退出；没有 IPC 父进程的独立启动仍受支持。

如必须手动处理无法读取的旧标记，应先停止所有 OpenGrove 进程，只删除错误中
明确列出的 `.lock` JSON 文件。不要使用通配符，也不要删除 `.lock.sqlite` 协调文件。
