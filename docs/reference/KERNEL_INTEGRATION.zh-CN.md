...
App 版本激活和主动恢复 App 默认设置可以重新应用 App 配置。普通同步也保留非 PM 内置员工已保存的权限；
App 没有声明权限档位时，保留该 App 员工已保存的权限。对没有支持声明的模型，Claude 缓存刷新或丢失
可能影响选项是否可用、以及新员工的默认档位，不改写已有选择；内核不支持的组合仍会修正。v3 迁移只补齐员工缺失的权限、
