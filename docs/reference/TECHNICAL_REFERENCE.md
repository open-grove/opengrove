...
Google AI Studio defaults to Gemini's native protocol for Pi, OpenCode and Kimi Code and also declares Google's official OpenAI Chat Completions endpoint. Adapters preserve the selected protocol when reusing model metadata. Pi and OpenCode expect the Google version prefix in the API base; Kimi's Google SDK appends it itself, so its adapter removes a trailing `/v1` or `/v1beta` while preserving custom proxy path prefixes. Use Kimi Code 0.37.0 or newer for Google tools: [upstream fixed lost thought signatures in that release](https://github.com/MoonshotAI/kimi-code/pull/2914). Existing Google settings with the former official bare-origin default are migrated without changing custom proxy addresses or credentials. Provider views include a read-only `bindings` map from Kernel id to the protocol selected by the Host; the UI does not independently infer supported routes.

OpenAI Chat Completions and Responses are distinct wire APIs. Explicit `wireApi` selections reach native Responses transports in Pi, OpenCode, Hermes and Kimi Code. Codex accepts Responses directly; its Chat route uses OpenGrove's additional Responses-to-Chat adapter. That adapter is not native Codex support and is not a complete compatibility promise: Gemini tool-call signatures and multimodal state are not currently preserved. A Provider's declared protocol and endpoint alone do not prove all of its tools or model capabilities work through an adapter.

Model limits are passed through native configuration boundaries: Kimi receives context and output limits through its shared completion budget, Hermes receives per-model context lengths, Codex receives the selected model's context window, and Claude Agent receives its context-window override. SDK usage reports may still use native defaults for other limits or unknown pricing; an unknown cost is not evidence that a request is free. Native session identities include the Provider route and wire API, but exclude the model catalog and its metadata. Catalog refreshes must not create new conversations. ACP model selection uses advertised `configOptions` and `session/set_config_option`, including aliases returned after session creation and loading. The older `session/set_model` method is used only when the Kernel does not advertise a model config option. An API model name is not assumed to be a configuration identifier.

The bundled catalog retains model limits, modalities, reasoning options and prices. Pi uses these facts for models absent from its native catalog, and OpenCode receives known capabilities without fabricated fixed limits. OpenAI, Anthropic, Gemini and DeepSeek model discovery refresh the available model list; Gemini discovery also retains input/output limits. Missing capability information remains absent from the catalog. Kernel SDK requirements may still require conservative runtime defaults for an unknown custom model.

An opt-in real API check is available after `npm run build:server`: run `node scripts/smoke-provider-routes.mjs --provider gemini` or `--provider deepseek` with the corresponding API key environment variable configured. Add `--kernel pi` or `--kernel opencode` to narrow the check, or `--settings /path/to/bridge-settings.json` to read a local configuration without modifying it. The check makes paid API calls and verifies a tool-call round trip using synthetic input and isolated temporary files. It is not part of unattended CI.
...
protected. This compatibility path can be removed when PID-only writers and
...
including parent loss before its lifecycle listeners were installed.
