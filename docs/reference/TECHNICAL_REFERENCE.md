...
Model limits are passed through native configuration boundaries: Kimi receives context/output limits, Hermes receives per-model context lengths, Codex receives the selected model's context window, and Claude Agent receives its context-window override. SDK usage reports may still use native defaults for other limits or unknown pricing; an unknown cost is not evidence that a request is free. Wire API and model metadata changes invalidate cached Host bindings. ACP model selection uses advertised `configOptions` and `session/set_config_option`, including aliases returned after session creation and loading. The older `session/set_model` method is used only when the Kernel does not advertise a model config option. An API model name is not assumed to be a configuration identifier.
