...
- Feature CSS stays beside its component. Composer controls respond to their own container, employee fields keep their existing form model, and compact dialogs constrain content to the usable visual viewport without changing desktop dialog overflow. Long forms and resource previews can opt into `mobilePresentation="page"`; short confirmations keep their dialog presentation.
...
Page navigation and detail history use React Router at desktop and compact widths. File selection/reveal has one history writer. Back/forward file changes preserve the current editor draft before applying a new selection; if durable backup fails, the editor stays open with an error and the URL is restored to its current file. The App Store publish form retains its leave confirmation on browser back/forward. Pane switches and Dialog subpages keep their own control semantics rather than adding a browser history entry for every focus change.
