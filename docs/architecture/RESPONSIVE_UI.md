...
Unread counts come from the existing Host-backed Rooms state. The compact chat tab shows the same App group unread count used by the main rail, plus the existing pending-action count. New messages do not change the selected workspace panel.

Page navigation and detail history use React Router. The App Store publish form retains its leave confirmation on browser back/forward. Peer tabs and Dialog subpages keep their own control semantics rather than adding a browser history entry for every focus change.
