...
Automatic formal-version switching rejects external `.git` files (including linked
worktrees) until the user relocates their repository: their shared repository path
registration cannot safely be copied to a new program generation. The existing
mount and Git connection remain intact.
