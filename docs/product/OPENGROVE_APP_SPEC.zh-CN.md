...
它不接收客户端 tgz 后重打包。管理员不勾选
“同时应用到我当前的 App”时，选中正式版本不变，草稿保留且发布基础推进到新版本；
勾选时，Host 激活刚发布的确切正式制品，完整采用其员工默认值、选中该正式版本并
关闭草稿。两条路径都保留本机 workspace 和 Room 历史。
...
每个已安装 App 都有一个本机选中的正式版本；新版本不会静默改变它。版本管理页读取
registry 的正式版本目录，并通过同一事务化安装接缝激活指定兼容版本。兼容性继续使用
