...
App 商店安装或本地导入成功时，App 根目录就已经是 Git 工作副本；初始程序源码保存点
尚未建立前，安装不得报告成功。Host 使用随应用提供的 Git 实现管理新建
App 的本机历史，不要求系统预装 `git`。Host 管理的 repository 元数据存放在本机 App
数据目录，新建 App 根目录只保留标准 `.git` 指针，因此 Codex、App 构建师和普通 Git
工具看到的是这个工作副本。如果导入目录已经是 Git 仓库或 linked worktree，它原有的
`.git`、HEAD、分支、暂存区、忽略规则和历史继续是权威，OpenGrove 不得改写；OpenGrove
为这个外部工作副本建立的发布保存点写入另一套仅限本机的私有 repository，不连接用户
分支或远端。Workspace、匹配仓库忽略规则且尚未被 Git 跟踪的文件、结构规则覆盖的凭据、
缓存和其他仅限本机的运行状态不进入保存点；外部仓库已经跟踪的文件继续遵循普通 Git
语义，不会因为后来又匹配忽略规则而被静默删除。点击“保存”会先为当前
程序源码建立明确保存点，再保存可恢复草稿；正式发布日志永久记录这次发布所绑定的
保存点 commit。版本管理页用产品语言显示程序源码是否还有未保存修改以及当前保存时间，
不向用户暴露 Git 实现术语。发布从该不可变保存点物化输入，因此此后在活动 App 中继续
编辑属于下一次保存，不能悄悄改变已经开始的发布。正式版本切换或重新打开草稿必须保留 `.git` 连接和 Workspace，不能用
商店包内的程序副本覆盖本机编辑历史。外部 `.git` 文件（包括 linked worktree）涉及共享仓库的路径登记，当前自动正式版本切换会明确拒绝，保留原挂载和 Git 连接；需由用户先完成仓库迁移。
正式版本激活成功后，Host 会把已安装的精确正式程序内容记录为新的程序源码保存点，
因此版本号、版本说明等发布期规范化字段不会让 Agent 一进入新版本就看到伪造的未保存修改。

正式发布时，Host 先建立不可变的程序源码保存点，并从中物化可恢复的 prebuild 草稿；存在本机发布构建配方时，Host 在
该精确草稿的隔离副本中执行 argv 命令，只事务式提升声明输出，再保存 postbuild
精确草稿。构建失败、取消、超时、选中保存点的物化内容变化或 App 安装代际变化均不得
创建远端任务；活动工作副本随后发生的无关编辑留在本机，不改变本次已选定的发布输入。Host
...
未修改的文本编辑器会跟随外部文件变化，不会把载入内容写回磁盘。用户输入会立即标记为
已修改，并保留编辑器的不可变文档快照。Markdown 全文转换和草稿备份在停止输入 500 毫秒后
合并执行；持续输入时，最多每两秒安排一次备份。Host 保存时检查读取时的内容版本，拒绝
过期写入。草稿使用 IndexedDB 保存，每条记录只包含一份草稿正文和基准版本号，按已确定的
Workspace、App 和文件路径隔离。编辑器等待 Workspace 身份及首次草稿恢复尝试完成。
如果无法读取备份存储，仍可编辑、按版本保存原文件及下载草稿；该编辑控制器只在内存中
保留新输入，不覆盖尚未读到的旧备份。未保存的输入仍受导航和关闭保护，保存原文件成功后
解除保护。保存后重新打开文件或刷新时可重新尝试恢复。损坏记录会以原子事务移入隔离区并
显示提示，用户可以继续编辑并备份新草稿；原始损坏值保留以便找回文字。隔离失败时保留原
记录并退回内存编辑。备份属于当前浏览器配置和来源，仍受本机存储可用性及配额限制。

草稿与磁盘都变化时，自动保存暂停。工作台提供左侧磁盘版本、右侧可编辑结果的手动比较，
允许复制单处改动、编辑结果后保存。保存会再次检查已审阅版本；外部再次修改后需要重新
审阅，不做自动文本合并。冲突和文件保存失败不阻止切换文件或页签，切换前会先提交最新
草稿备份。仅在备份失败时保留当前页面并显示错误及下载入口。关闭或刷新时，如果最新
备份或备份删除仍未提交，会提示用户。放弃草稿会采用当前磁盘内容并删除备份，不写入原文件。
当前打开的文件每三秒检查一次变化，编辑期间也继续检查；即使内容未变，也会读取文本
（上限 2 MB）并计算内容哈希，而非仅检查文件属性。

完整文本的 `workspace.read` 返回内容 `revision`；覆盖文件时，`workspace.write` 必须把
该值作为 `expectedRevision` 传回。省略只允许新建，`"missing"` 明确要求文件不存在。
覆盖已有文件却未传版本会返回 `workspace_file_revision_required`（428）；版本过期返回
`workspace_file_conflict`（409）。调用方需要重新读取、检查改动后再重试。原生 Kernel
工具和 App CLI 仍是外部文件写入者，Host 的检查不能阻止它们绕过该协议直接覆盖磁盘。
