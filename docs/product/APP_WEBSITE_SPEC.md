...
Login implements first-party OAuth 2.0 authorization code with PKCE and a profile endpoint, not OpenID Connect. Website cookies contain only encrypted, short-lived delegated credentials. Tokens use a separate audience so account-only services cannot interpret them as unrestricted account credentials.
